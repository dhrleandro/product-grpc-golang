...
func TestProductToJsonDTO(t *testing.T) {
	p, _ := dto.NewProductDTO(5, "Computer", "Core i7", 5348)
	json, err := p.ToJson()

	require.Equal(t, err, nil)

	testJson := []byte(`{"id":5,"name":"Computer","description":"Core i7","price":5348}`)
	require.Equal(t, json, testJson)
}

func TestNewProductFromJsonDTO(t *testing.T) {
	json := []byte(`{"id":58,"name":"Computer","description":"Core i7","price":100}`)
